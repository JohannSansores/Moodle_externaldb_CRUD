...
class StoreExternalUserRequest extends FormRequest
{
    ...
    {
        return [
            'username'       => 'required|string|max:100|unique:moodle_usuarios,username',
            'password'       => 'required|string|min:8',
            'firstname'      => 'required|string|max:100',
            'lastname'       => 'required|string|max:100',
            'email'          => 'required|email|max:150|unique:moodle_usuarios,email',
            'curp'           => 'nullable|string|size:18|unique:moodle_usuarios,curp|regex:/^[A-Z]{4}[0-9]{6}[HM][A-Z]{5}[0-9A-Z]{2}$/',
